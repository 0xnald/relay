"""Shared narrow AgentCore intake implementation."""
...
async def handle_agentcore_intake(
    ...
    result = await IntakeAgentService(AgentFactory(active)).interpret(
        request.message, trace_id=trace_id, actor_identity=request.source
    )
